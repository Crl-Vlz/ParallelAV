...
__device__ bool device_is_valid(int* board, int row, int col, int n) {
...
/// <summary>
/// A kernel that tries to solve the sudoku boar dvia recursive backtracking
/// Check if this method works in GPU, if not look for another
/// </summary>
/// <param name="board">1D array representing the sudoku board</param>
/// <param name="found">Was a solution found</param>
/// <returns>Void</returns>
__global__ void device_solve_sudoku(int* board, bool* found) {
...
int main()
{
    ...
    cudaMemcpy(host_pointer, device_pointer, sizeof(int) * 81, cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();
